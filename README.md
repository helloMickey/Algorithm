...
- 字符串表达式计算：
  ...
  中缀表达式转后缀表达式（**逆波兰表达式**）。 [例1](lc-cn/基本计算器.cpp)， [例2](lc-cn/基本计算器2.cpp)， 符号栈是否进行弹出取决于，当前操作符与栈顶操作符优先谁高，栈顶优先级高或者相等则先进行出栈和计算（相同优先级别，从左到右计算， 所以也要弹出），直到栈顶元素优先机低于当前操作符
...
- [一篇文章解决所有二叉树路径问题（问题分析+分类模板+题目剖析）](https://leetcode-cn.com/problems/path-sum-iii/solution/yi-pian-wen-zhang-jie-jue-suo-you-er-cha-smch/) 🚩
...
二分查找思想：二分查找适合在有序的数组种查找某一个值，或者某一个**边界**！

- [在排序数组中查找元素的第一个和最后一个位置](lc-plug/34.在排序数组中查找元素的第一个和最后一个位置.py)
- [有序矩阵中第K小的元素](lc-cn/有序矩阵中第K小的元素.cpp) 

搜索相关题目：

- [搜索二维矩阵](lc-cn/搜索二维矩阵.cpp)，[搜索二维矩阵2](lc-cn/搜索二维矩阵2.cpp)
...
- 快排实现 ： [雪糕的最大数量](lc-cn/雪糕的最大数量.cpp)
  ...
  void quick_sort(vector<int>& nums, int i, int j) {
      ...
      int pivot = nums[high] > pivot_num ? high-1 : high;
...
- 基于邻接表的 DFS 遍历 👉 [传递信息](lc-cn/传递信息.cpp) ，[除法求值](lc-cn/除法求值.cpp)
...
- 背包问题：1）01背包；2）完全背包；3）多重背包问题；
    ```C++
    // 背包问题中，dp[i][j]表示： 前 i 个物品中，容量为 j 的背包所能获得的物品总价值
...
- [编辑距离](lc-cn/编辑距离.cpp)
- 一般当我们实现 **DFS+打表** （也称 `记忆化搜索`）的方式之后，很容易得到其基于迭代的 **动态规划** ：[交错字符串](lc-cn/交错字符串.cpp) 、 [分割等和子集](lc-cn/分割等和子集.cpp)
...
- [最佳买卖股票时机含冷冻期](lc-cn/最佳买卖股票时机含冷冻期.cpp)
...
- 🚩 通过DFA（有限状态自动机）实现 [字符串转整数](https://leetcode-cn.com/problems/string-to-integer-atoi/solution/zi-fu-chuan-zhuan-huan-zheng-shu-atoi-by-leetcode-/)。
  ...
  [表示数值的字符串](lc-cn/剑指offer/表示数值的字符串.cpp)
